...
set SCRIPT_DIR=%~dp0
:: Always run from our own directory: the script may be launched from any cwd
:: (cmd inherits the caller's directory), and the final tar step is
:: cwd-relative — a foreign cwd would silently pack another tree's package/.
cd /d "%SCRIPT_DIR%"
...
:: Aggregate the 17 bridge plugins into the package so the HAR is a
